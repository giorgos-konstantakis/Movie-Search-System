...
function NowPlayingMoviesReverse(props) {
    ...
    const firstPage = () => {
        return (
            <div className="text-center col-md-12">
                ...
                <Link className="card-header-font-sz" to={`/movies/now_playing_movies/page/${nowPlayingMovies.page + 3}`}>
    ...
    const renderPages = () => {
        return (
            <div className="text-center col-md-12">
                <span className="mr-5 card-header-font-sz text-white">Results: {(nowPlayingMovies.page - 1) * 20 + 1} - {nowPlayingMovies3.page * 20}</span>
                <Link className="mr-5 card-header-font-sz" to={`/movies/now_playing_movies/page/${nowPlayingMovies.page - 3}`}>
                    <button type="button" className="btn btn-outline-dark card-header-font-sz text-white p-2">
                        <i class="fas fa-angle-double-left mr-2"></i>Previous Page
            </button>
                </Link>
                        ...
                        {nowPlayingMovies.results && nowPlayingMovies.results.map((nowPlaying, i) => {
                            ...
                            return (
                                <div key={i} className="my-1 py-2">
                                    <Link className="text-light link-underline-style name-title-font-sz" to={`/movie_info/${nowPlaying.id}`}><img className="photo-opac" src={`https://image.tmdb.org/t/p/w45/${nowPlaying.poster_path}`} alt="new" /> {nowPlaying.title}</Link>
                                </div>
                            )
                        }
                        )}
